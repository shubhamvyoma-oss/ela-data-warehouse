...
`catalogue` collector (the port of `Attendance data/build_course_catalog.py`, Stage 1 of the
...
4. For each remaining batch, calls `/masterbatch/{batch_id}` and writes the resolved row(s)
   immediately via `CollectorRuntime.commit_rows(...)`, updating the checkpoint
