        ...
        for job in jobs:
            cursor.execute(
                ...
                INSERT INTO system.scheduler_jobs (
                    job_name, collector_name, enabled, interval_minutes, next_run_at, updated_at
                ...
                (
                    job["name"],
                    job["collector"],
                    globally_enabled and job["enabled"],
    ...
    with database.transaction() as connection, connection.cursor() as cursor:
        cursor.execute(
            ...
            WITH candidate AS (
                ...
                WHERE enabled = true AND COALESCE(next_run_at, '-infinity') <= now()
