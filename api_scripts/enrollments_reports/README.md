...
`build_chunks(start_date, end_date, chunk_days)` in `collector.py` splits that inclusive
...
`COLUMNS` in `collector.py` is a direct 1:1 port of `FIELDS` from the original
